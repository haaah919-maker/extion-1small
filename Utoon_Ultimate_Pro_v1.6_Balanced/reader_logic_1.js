    ...
    document.getElementById('btn-pdf').onclick = async () => {
        ...
        try {
            ...
            for (let i = 0; i < images.length; i++) {
                ...
                btn.innerHTML = `⏳ ${i+1}/${images.length} - أعلى دقة`;

                // أعلى دقة: نحمل الصورة ونحولها لـ PNG بدون ضغط
                const response = await fetch(imgEl.src, {mode: 'cors'});
                const blob = await response.blob();
                
                // نستخدم createImageBitmap للحفاظ على الدقة الأصلية 100%
                const bitmap = await createImageBitmap(blob);
                const canvas = document.createElement('canvas');
                canvas.width = bitmap.width;
                canvas.height = bitmap.height;
                const ctx = canvas.getContext('2d', { willReadFrequently: false, alpha: false });
                ctx.imageSmoothingEnabled = false; // ماكو تنعيم - دقة أصلية
                ctx.drawImage(bitmap, 0, 0);
                
                // PNG = بدون فقدان جودة إطلاقاً
                const dataUrl = canvas.toDataURL('image/png', 1.0);
                const w = bitmap.width;
                const h = bitmap.height;
                bitmap.close();

                // صفحة بحجم الصورة بالضبط - ماكو حواف بيضاء نهائياً
                const orientation = w > h ? 'landscape' : 'portrait';
                
                if (i === 0) {
                    // compress: false = أعلى جودة، hotfixes للـ px الدقيق
                    pdf = new jsPDF({ 
                        orientation, 
                        unit: 'px', 
                        format: [w, h], 
                ...
                // نضيف الصورة 100% بدون أي ضغط - NONE = أعلى جودة
            ...
            pdf.setProperties({
                ...
                subject: 'Manga Chapter - Highest Quality',
                author: 'Utoon Ultimate Pro',
                creator: 'Utoon Ultimate Pro v1.5 HQ'
            });

            pdf.save(`${mangaSlug}_${chapterSlug}_ULTRA_HD.pdf`);
        } catch (err) {
            console.error(err);
