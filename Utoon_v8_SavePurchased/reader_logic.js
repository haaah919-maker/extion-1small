async function injectReader() {
    ...
    // ---- API-based image fetching (works for locked chapters) ----
    async function fetchImagesFromAPI() {
        try {
            ...
            // Match chapter by slug or number
    ...
    // Fallback: guess CDN URLs (old method)
    function buildFallbackUrls() {
        const list = [];
        document.querySelectorAll('img').forEach(i => {
            const s = i.src || i.dataset.src;
            if (s && s.includes(baseUrl.split('/')[2])) list.push(s);
        });
        for (let i = 1; i <= 300; i++) {
            const n = i.toString().padStart(2, '0');
            list.push(`${baseUrl}/${mangaSlug}/${chapterSlug}/${n}.jpg`);
            list.push(`${baseUrl}/${mangaSlug}/${chapterSlug}/${n}.webp`);
        }
        return [...new Set(list)];
    }

    // Placeholder — real images loaded after HTML inject
    let imageUrls = [];
    ...
    document.body.innerHTML = `
        <div id="u-reader-main" style="background-color: #0a1014; color: #e2d9f3; display: flex; flex-direction: column; min-height: 100vh; position: relative; z-index: 9999999; padding: 0; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; box-sizing: border-box; align-items: center; transition: 0.5s; overflow-x: hidden;">
            ...
            <div id="img-container" style="max-width:850px; width:100%; margin:30px 0; background: rgba(10, 10, 15, 0.9); box-shadow: 0 0 60px rgba(0,0,0,0.7); border: 1px solid rgba(91, 33, 182, 0.2); border-radius: 12px; overflow: hidden; position: relative;">
                <div id="api-loading" style="text-align:center; padding:60px 20px; color:#a78bfa;">
                    <div style="font-size:32px; margin-bottom:15px;">🔄</div>
                    <div style="font-size:15px; font-weight:bold;">جاري تحميل الصور...</div>
                    <div style="font-size:12px; color:#6d28d9; margin-top:8px;">يتم جلب الصور من API</div>
                </div>
            </div>
    ...
    // Effect Logic
    // ---- Load images via API (async, after HTML is injected) ----
    (async () => {
        const container = document.getElementById('img-container');
        const loader    = document.getElementById('api-loading');

        // Step 1: try REST API (handles locked/premium chapters)
        let apiImgs = await fetchImagesFromAPI();

        if (apiImgs && apiImgs.length > 0) {
            imageUrls = apiImgs;
        } else {
            // Step 2: fallback to CDN guessing
            imageUrls = buildFallbackUrls();
        }
        ...
        imageUrls.forEach(src => {
            ...
            img.src  = src;
            img.style.cssText = 'width:100%; display:block; min-height:200px; background:#1a1a1a url("https://i.gifer.com/ZZ5H.gif") center no-repeat; background-size:50px;';
            img.onerror = () => img.remove();
            container.appendChild(img);
        });

        // Show message if no images loaded after a delay
        setTimeout(() => {
            if (container.querySelectorAll('img').length === 0) {
                container.innerHTML = '<div style="text-align:center;padding:60px;color:#ef4444;font-size:15px;">⚠️ لم يتم العثور على صور لهذا الفصل</div>';
            }
        }, 12000);
    })();
