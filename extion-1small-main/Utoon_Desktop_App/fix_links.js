(function() {
    ...
    function initBulkDownload() {
        ...
        // Floating Button: Pro Dashboard
        const dashBtn = document.createElement('button');
        dashBtn.id = 'u-dash-btn';
        dashBtn.innerHTML = '💎 Pro Dashboard';
        dashBtn.style.cssText = 'position:fixed; bottom:20px; right:190px; z-index:999999; background:linear-gradient(135deg,#a855f7,#7c3aed); color:white; border:none; padding:12px 24px; border-radius:50px; font-weight:bold; cursor:pointer; box-shadow:0 5px 15px rgba(0,0,0,0.5); font-size:14px; transition:0.3s;';
        dashBtn.onmouseover = () => dashBtn.style.transform = 'scale(1.05)';
        dashBtn.onmouseout = () => dashBtn.style.transform = 'scale(1)';
        dashBtn.onclick = () => {
            if (typeof showProDashboard === 'function') {
                showProDashboard();
            } else {
                alert("Loading dashboard script...");
            }
        };
