(async function() {
    ...
    // Better locked chapter detection: Check for premium-lock indicators on the page
    let isLockedOnSite = false;
    const premiumLock = document.querySelector('.premium-lock, .wp-manga-chapter.premium-block, .chapter-locked, .locked-content');
    const hasNoImages = (!imageUrls || imageUrls.length === 0);
    const hasPremiumText = document.body.innerText.includes('premium') || document.body.innerText.includes('unlock') || document.body.innerText.includes('subscribe');
    
    // Chapter is considered locked if it has premium lock indicators OR no images AND premium text
    isLockedOnSite = (premiumLock !== null) || (hasNoImages && hasPremiumText);
        ...
        if (!status.allowed) {
            document.body.innerHTML = `
                <div style="background:#0a0514; color:white; height:100vh; display:flex; flex-direction:column; align-items:center; justify-content:center; font-family:sans-serif; text-align:center; padding:20px;">
                    <h1 style="color:#7c3aed; font-size:40px; margin-bottom:10px;">Daily Limit Reached 🛑</h1>
                    <p style="font-size:20px; max-width:600px; line-height:1.6; color:#94a3b8;">
                        You have used your 5 free locked chapters for today.<br>
                        To read unlimited chapters with <b>No Ads</b> and <b>Instant Access</b>, upgrade to VIP Premium!
                    </p>
                    <div style="margin:30px 0; display:flex; gap:15px;">
                        <button onclick="window.location.href='${mangaMainUrl}'" style="background:#1e293b; color:white; border:1px solid #475569; padding:12px 25px; border-radius:8px; cursor:pointer; font-weight:bold;">Return to Manga</button>
                        <button onclick="window.open('https://discord.gg/utoon', '_blank')" style="background:linear-gradient(135deg,#a855f7,#7c3aed); color:white; border:none; padding:12px 30px; border-radius:8px; cursor:pointer; font-weight:bold; box-shadow:0 5px 15px rgba(124,58,237,0.3);">Get VIP Key (5$)</button>
                    </div>
                    <div style="color:#64748b; font-size:14px;">
                        Owner: <span style="color:#a78bfa;">Hpwin</span> | Support: <a href="https://discord.gg/utoon" style="color:#a78bfa; text-decoration:none;">Join Discord</a>
                    </div>
                </div>
            `;
            return;
    ...
    document.body.innerHTML = `
        <div id="u-reader-main" style="background-color:#0a1014;color:#e2d9f3;display:flex;flex-direction:column;min-height:100vh;position:relative;z-index:9999999;padding:0;font-family:'Segoe UI',Tahoma,Geneva,Verdana,sans-serif;align-items:center;overflow-x:hidden;">
            ...
            <div id="header-controls" style="position:sticky;top:0;width:100%;background:rgba(10,5,25,0.95);padding:12px 16px;border-bottom:2px solid #a855f7;z-index:10000001;display:flex;align-items:center;justify-content:center;gap:10px;flex-wrap:wrap;backdrop-filter:blur(10px);">
                <div id="user-status" style="background:rgba(168,85,247,0.15);border:1px solid rgba(168,85,247,0.4);padding:6px 12px;border-radius:6px;font-size:12px;font-weight:600;color:#a78bfa;white-space:nowrap;">Loading...</div>
            ...
            <div id="img-container" style="max-width:850px;width:100%;margin:20px auto;background:rgba(0,0,0,0.8);box-shadow:0 0 50px rgba(0,0,0,0.8);border-radius:12px;overflow:hidden;">
                ...
                <!-- Discord Footer Section -->
                <div id="u-discord-footer" style="background:#130a2a; padding:40px 20px; text-align:center; border-top:2px solid #7c3aed;">
                    <h3 style="color:#a78bfa; margin-bottom:10px;">Enjoying the Reader?</h3>
                    <p style="color:#94a3b8; font-size:14px; margin-bottom:20px;">Join our community for updates, requests, and support!</p>
                    <a href="https://discord.gg/utoon" target="_blank" style="display:inline-block; background:#5865F2; color:white; text-decoration:none; padding:12px 30px; border-radius:8px; font-weight:bold; transition:0.3s;">Join Discord Support</a>
                    <div style="margin-top:20px; font-size:12px; color:#475569;">
                        Owner: <b>Hpwin</b> | Utoon Ultimate Pro v3.0
                    </div>
                </div>
    ...
    async function updateUserStatus() {
        ...
        if (typeof getUserStatus !== 'undefined') {
            try {
                ...
                if (status.isPremium) {
                    ...
                    statusEl.style.borderColor = 'rgba(168,85,247,0.4)';
            ...
            statusEl.textContent = 'Status: Free Mode';
        }
    }
