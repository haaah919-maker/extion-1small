async function injectReader() {
    ...
    document.body.innerHTML = `
        <div id="u-reader-main" style="background-color:#0a1014;color:#e2d9f3;display:flex;flex-direction:column;min-height:100vh;position:relative;z-index:9999999;padding:0;">
            ...
            <div id="header-controls" style="position:sticky;top:0;width:100%;background:rgba(10,5,25,0.95);padding:12px 16px;border-bottom:2px solid #a855f7;z-index:10000001;display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap;">
                ...
                <div style="display:flex;align-items:center;gap:5px;background:rgba(34,197,94,0.08);padding:6px 10px;border-radius:6px;border:1px solid #22c55e;">
                    <select id="autoscroll-select" style="background:transparent;color:#22c55e;border:none;cursor:pointer;font-size:12px;outline:none;font-weight:600;">
                        <option value="0">⏸️ إيقاف</option>
                        <option value="0.3">🐢 بطيء</option>
                        <option value="0.5" selected>▶️ عادي</option>
                        <option value="1">⚡ سريع</option>
                        <option value="2">🚀 سريع جدا</option>
                    </select>
                </div>
    ...
    // تحكم التمرير التلقائي
    const autoSelect = document.getElementById('autoscroll-select');
    if (autoSelect) {
        autoSelect.onchange = (e) => {
            ...
            if (chrome && chrome.storage) chrome.storage.local.set({ autoScrollSpeed });
        };
        // حمل السرعة المحفوظة
        chrome.storage.local.get(['autoScrollSpeed'], (res) => {
            if (res.autoScrollSpeed !== undefined) {
                autoScrollSpeed = res.autoScrollSpeed;
                autoSelect.value = autoScrollSpeed;
    ...
    document.getElementById('btn-pdf').onclick = async () => {
        ...
        try {
            ...
            for (let i = 0; i < images.length; i++) {
                ...
                btn.innerHTML = `⏳ ${i+1}/${images.length}`;

                // نحمل الصورة بأعلى جودة ممكنة مع حجم معقول
                ...
                // JPEG 92% = جودة ممتازة وحجم 10-30 ميجا (مو 200!)
                ...
                // ماكو حواف بيضاء - الصورة تغطي 100%
                pdf.addImage(dataUrl, 'JPEG', 0, 0, w, h, '', 'SLOW');
            }

            pdf.setProperties({
                title: `${mangaSlug} - ${chapterSlug}`,
                subject: 'Manga Chapter',
                author: 'Utoon Ultimate Pro',
                creator: 'Utoon Ultimate Pro v1.6'
            });
